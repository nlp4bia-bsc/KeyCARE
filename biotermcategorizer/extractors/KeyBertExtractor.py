...
class KeyBertExtractor(Extractor):
    def __init__(self, language, max_tokens, pos, pos_pattern):
        """
    ...
    def extract_terms(self, text):
        ...
        keywords_all = list()
        if self.pos:
            keywords_all = self.extractor.extract_keywords(text, vectorizer=KeyphraseCountVectorizer(spacy_pipeline="es_core_news_sm", stop_words='spanish', pos_pattern=self.pos_pattern), **filtered_kwargs)
        else:
